...
import {
  ADDRESS_ADMIN_SELECT,
  AFFILIATION_ROW_SELECT,
  findAvailableAddressId,
  getAddressAdminById,
  isAddressOccupied,
  isRegionValidForAddressType,
  listRegionsForAddressType,
  type AddressType as AffAddressType,
} from "./affiliationAddress.js";
import {
  materialFromBody,
...
function collectAffiliationDraftUpdates(
  b: Record<string, unknown>,
): { fields: string[]; vals: unknown[]; error?: string } {
  const fields: string[] = [];
  const vals: unknown[] = [];
  const allowedText = ["applicant_name", "applicant_dept", "notes", "group_name"] as const;
  ...
  if (b.requested_address_region !== undefined) {
    ...
    if (!reg) return { fields, vals, error: "地址区域不能为空" };
...
api.get("/address-regions", (req, res) => {
  ...
  res.json({ regions: listRegionsForAddressType(addressType as AffAddressType) });
});

api.get("/addresses", requireAdmin, (req, res) => {
  const addressType = req.query.address_type as string | undefined;
  const q = (req.query.q as string | undefined)?.trim();
  const occupancy = req.query.occupancy as string | undefined;
...
api.post("/addresses/import", requireAdmin, (req, res) => {
  ...
  type Norm = { address_type: AddressType; address_region: string; detail_address: string };
  const normalized: Norm[] = [];
  const details: { row: number; message: string }[] = [];
  for (let i = 0; i < raw.length; i++) {
    const o = raw[i];
    const obj = typeof o === "object" && o !== null ? (o as Record<string, unknown>) : null;
...
api.delete("/addresses/:id", requireAdmin, (req, res) => {
  ...
  if (!exists) return res.status(404).json({ error: "未找到地址记录" });
...
api.get("/users", requireAdmin, (_req, res) => {
  const rows = db
    .prepare(
      `SELECT id, username, role, display_name, oa_member_id, created_at FROM users ORDER BY role DESC, username COLLATE NOCASE`,
    )
    .all();
  res.json(rows);
});
...
api.patch("/users/:id", requireAdmin, (req, res) => {
  ...
  const oa_member_id =
    b.oa_member_id === null || b.oa_member_id === ""
      ? null
...
api.post("/affiliations", (req, res) => {
  ...
  if (!isRegionValidForAddressType(b.requested_address_type as AffAddressType, requestedRegion)) {
  ...
  const reqType = b.requested_address_type as AffAddressType;
...
api.patch("/affiliations/:id", (req, res) => {
  ...
  const assertRequestedRegion = (row: { requested_address_type: string; requested_address_region: string }) => {
    if (!isAddressType(row.requested_address_type)) return "地址类型无效";
    if (!isRegionValidForAddressType(row.requested_address_type as AffAddressType, row.requested_address_region)) {
  ...
  if (b.action === "submit" && cur.status === "draft") {
    ...
    const full = db.prepare("SELECT * FROM affiliation_requests WHERE id = ?").get(id) as Record<string, unknown> &
      { requested_address_type: string; requested_address_region: string };
    const regionErr = assertRequestedRegion(full);
    if (regionErr) return res.status(400).json({ error: regionErr });
    ...
    db.prepare(
      `UPDATE affiliation_requests SET status = 'pending', submitted_at = ?, updated_at = ?,
       address_id = NULL, reviewer_name = NULL, review_comment = NULL, reviewed_at = NULL WHERE id = ?`,
    ).run(t, t, id);
    ...
    if (b.action === "approve") {
      const full = db.prepare("SELECT * FROM affiliation_requests WHERE id = ?").get(id) as
        | {
            requested_address_type: string;
            requested_address_region: string;
          }
        | undefined;
      if (!full) return res.status(404).json({ error: "未找到申请" });
      const reqType = full.requested_address_type as AffAddressType;
      ...
      db.prepare(
        `UPDATE affiliation_requests SET status = ?, address_id = ?, reviewer_name = ?, review_comment = ?,
    ...
    const prevApproved =
      isAdmin && cur.status === "approved"
        ? (db.prepare("SELECT requested_address_type, requested_address_region FROM affiliation_requests WHERE id = ?").get(
            id,
          ) as { requested_address_type: string; requested_address_region: string })
        : null;
    ...
    if (prevApproved && (b.requested_address_type !== undefined || b.requested_address_region !== undefined)) {
      const full = db.prepare("SELECT requested_address_type, requested_address_region FROM affiliation_requests WHERE id = ?").get(
        id,
      ) as { requested_address_type: string; requested_address_region: string };
      const regionErr = assertRequestedRegion(full);
      if (regionErr) return res.status(400).json({ error: regionErr });
      const changed =
        full.requested_address_type !== prevApproved.requested_address_type ||
...
api.get("/stats", (req, res) => {
  ...
  const pendingAgreementApprovals = db
    .prepare(
      "SELECT COUNT(*) AS c FROM affiliation_requests WHERE agreement_status = 'pending' AND created_by_user_id = ?",
    )
    .get(uid!) as { c: number };
