...
const addrTable = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='addresses'").get() as
  | { name: string }
  | undefined;
if (addrTable) {
...
{
  const names = new Set(
    (db.prepare("PRAGMA table_info(affiliation_requests)").all() as { name: string }[]).map((c) => c.name),
  );
...
/** 挂靠申请：业务员只填类型/区域，审批后分配 address_id；将 address_id 改为可空 */
{
  ...
  const cols = db.prepare("PRAGMA table_info(affiliation_requests)").all() as ColInfo[];
  ...
  if (!hasRequested || addrNotNull) {
    ...
    db.exec(
      "CREATE INDEX IF NOT EXISTS idx_affiliation_requested ON affiliation_requests(requested_address_type, requested_address_region)",
    );
  }
}
...
if (userCount === 0) {
  ...
  ).run("user_admin", "admin", bcrypt.hashSync("admin123", rounds), "admin", "系统管理员", t);
  db.prepare(
    `INSERT INTO users (id, username, password_hash, role, display_name, created_at) VALUES (?,?,?,?,?,?)`,
...
if (count === 0) {
  ...
  db.prepare(
    `INSERT INTO addresses (id, address_type, address_region, detail_address, created_at, updated_at)
     VALUES (?,?,?,?,?,?)`,
  ).run(a2, "coworking", "上海市静安区", "南京西路1266号恒隆广场二期共享工位区", t, t);
  db.prepare(
    `INSERT INTO addresses (id, address_type, address_region, detail_address, created_at, updated_at)
     VALUES (?,?,?,?,?,?)`,
  ).run(a3, "business_secretary", "江苏省苏州市工业园区", "星湖街328号创意产业园商务秘书托管席位", t, t);
  const r1 = "aff_demo_1";
  db.prepare(
    ...
    "渠道李",
    "13800002222",
    "https://example.com/id-front-demo.jpg",
    "https://example.com/id-back-demo.jpg",
    "法人张",
    "13900003333",
    "上海市浦东新区张江路1号",
    "zhang@example.com",
