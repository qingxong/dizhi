...
export function getAddressAdminById(id: string): Record<string, unknown> | undefined {
  return db.prepare(`${ADDRESS_ADMIN_SELECT} WHERE a.id = ?`).get(id) as Record<string, unknown> | undefined;
}

export function isAddressOccupied(addressId: string): boolean {
  const row = db
    .prepare(
      `SELECT 1 AS x FROM affiliation_requests WHERE address_id = ? AND status = 'approved' LIMIT 1`,
    )
    .get(addressId) as { x: number } | undefined;
  return !!row;
}

/** 从地址库选取尚未被「已通过」申请占用的地址（同类型、同区域，先到先得） */
export function findAvailableAddressId(type: AddressType, region: string): string | null {
  ...
  const row = db
    ...
    .get(type, regionTrim) as { id: string } | undefined;
  return row?.id ?? null;
}

export function isRegionValidForAddressType(type: AddressType, region: string): boolean {
  const regionTrim = region.trim();
  if (!regionTrim) return false;
  return listRegionsForAddressType(type).includes(regionTrim);
}

export function listRegionsForAddressType(type: AddressType): string[] {
  const rows = db
    .prepare(
      `SELECT DISTINCT TRIM(address_region) AS address_region
       FROM addresses WHERE address_type = ?
       ORDER BY address_region COLLATE NOCASE`,
    )
    .all(type) as { address_region: string }[];
