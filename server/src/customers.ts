...
export type CustomerType = "channel" | "direct";
...
function normCompany(v: string | null): string {
...
export function customerDisplayName(row: CustomerRow): string {
  ...
  if (name && id) return `${name}（${id.slice(0, 4)}****${id.slice(-4)}）`;
  return name || id || "直客";
}

type CustomerBodyFields = Omit<
  CustomerRow,
  "id" | "created_by_user_id" | "created_at" | "updated_at" | "owner_display_name" | "oa_entry_id" | "oa_customer_sn" | "source"
>;

function rowFromBody(b: Record<string, unknown>): CustomerBodyFields {
  const ct: CustomerType = b.customer_type === "channel" ? "channel" : "direct";
...
export function validateCustomerFields(
  row: CustomerBodyFields,
  opts?: { fromOa?: boolean },
): string | null {
  if (opts?.fromOa) {
    if (row.customer_type === "channel") {
      if (
        !nonempty(row.channel_company_name) &&
    ...
    if (row.legal_phone && !isValidCnPhone(row.legal_phone)) return "法人手机号格式不正确";
    ...
    if (row.legal_email && !isValidEmail(row.legal_email)) return "法人邮箱格式不正确";
    return null;
  }

  if (row.customer_type === "channel") {
    if (!nonempty(row.channel_common_contact_name)) return "请填写渠道常用联系人姓名";
    if (!nonempty(row.channel_common_contact_phone)) return "请填写渠道常用联系人电话";
    if (!nonempty(row.channel_backup_contact_name)) return "请填写渠道备用联系人姓名";
    if (!nonempty(row.channel_backup_contact_phone)) return "请填写渠道备用联系人电话";
    if (row.channel_common_contact_phone && !isValidCnPhone(row.channel_common_contact_phone)) {
      return "渠道常用联系人电话格式不正确";
    }
    if (row.channel_backup_contact_phone && !isValidCnPhone(row.channel_backup_contact_phone)) {
  ...
  if (!nonempty(row.legal_name)) return "请填写法人姓名";
  if (!nonempty(row.legal_id_number)) return "请填写法人身份证号";
  if (!nonempty(row.legal_phone)) return "请填写法人手机号";
  if (!nonempty(row.legal_contact_address)) return "请填写法人联系地址";
  if (!nonempty(row.legal_email)) return "请填写法人邮箱";
...
function findChannelDuplicate(company: string | null, phone: string | null, excludeId?: string): CustomerRow | undefined {
  if (!phone) return undefined;
  const nc = normCompany(company);
  const np = normalizePhone(phone);
  const rows = db.prepare("SELECT * FROM customers WHERE customer_type = 'channel'").all() as CustomerRow[];
  return rows.find((r) => {
...
function findDirectDuplicate(idNumber: string | null, excludeId?: string): CustomerRow | undefined {
  if (!idNumber) return undefined;
  const nid = normId(idNumber);
  const rows = db.prepare("SELECT * FROM customers WHERE customer_type = 'direct'").all() as CustomerRow[];
  return rows.find((r) => {
    if (excludeId && r.id === excludeId) return false;
...
function duplicateMessage(existing: CustomerRow, isAdmin: boolean): string {
...
function canAccessRow(req: Request, row: CustomerRow): boolean {
  if (req.session.role === "admin") return true;
  return row.created_by_user_id === req.session.userId;
}

function enrich(row: CustomerRow): CustomerRow & { display_name: string } {
  return { ...row, display_name: customerDisplayName(row) };
}

function searchWhere(q: string): { sql: string; params: string[] } {
  const like = `%${q}%`;
  return {
    sql: ` AND (
...
export function registerCustomerRoutes(api: Router) {
  api.get("/customers", (req, res) => {
    ...
    const rows = (db.prepare(sql).all(...params) as CustomerRow[]).map(enrich);
    res.json(rows);
  });
  ...
  api.post("/customers/sync-from-oa", async (req, res) => {
    ...
    const userRow = db.prepare("SELECT oa_member_id FROM users WHERE id = ?").get(uid) as
  ...
  api.get("/customers/check-duplicate", (req, res) => {
    ...
    if (!ct) return res.status(400).json({ error: "请指定 customer_type" });
    ...
    if (ct === "channel") {
      ...
      if (!phone.trim()) return res.json({ duplicate: false });
    ...
    if (!idNo.trim()) return res.json({ duplicate: false });
  ...
  api.get("/customers/:id", (req, res) => {
    ...
    if (!row) return res.status(404).json({ error: "未找到客户" });
    if (!canAccessRow(req, row)) return res.status(403).json({ error: "无权查看该客户" });
    res.json(enrich(row));
  });
  ...
  api.post("/customers", (req, res) => {
    const uid = req.session.userId;
    if (!uid) return res.status(401).json({ error: "未登录" });
    const body = rowFromBody(req.body as Record<string, unknown>);
    ...
    if (body.customer_type === "channel") {
      ...
      if (dup) return res.status(409).json({ error: duplicateMessage(dup, isAdmin), existing_id: dup.id });
    } else {
    ...
    db.prepare(
      ...
      id,
      body.customer_type,
      body.channel_company_name,
      body.channel_common_contact_name,
      body.channel_common_contact_phone,
      body.channel_backup_contact_name,
      body.channel_backup_contact_phone,
      body.legal_name,
      body.legal_id_number,
      body.legal_phone,
      body.legal_contact_address,
      body.legal_email,
      body.enterprise_backup_name,
      body.enterprise_backup_phone,
      null,
      null,
      "local",
      uid,
    ...
    const row = db.prepare(`${CUSTOMER_SELECT} WHERE c.id = ?`).get(id) as CustomerRow;
    res.status(201).json(enrich(row));
  });

  api.patch("/customers/:id", (req, res) => {
    const id = String(req.params.id ?? "");
    const cur = db.prepare("SELECT * FROM customers WHERE id = ?").get(id) as CustomerRow | undefined;
    if (!cur) return res.status(404).json({ error: "未找到客户" });
    if (!canAccessRow(req, cur)) return res.status(403).json({ error: "无权修改该客户" });

    const b = req.body as Record<string, unknown>;
    const pick = (k: keyof CustomerRow) => (b[k] !== undefined ? b[k] : cur[k]);
    ...
    db.prepare(
      ...
      merged.channel_company_name,
      merged.channel_common_contact_name,
      merged.channel_common_contact_phone,
      merged.channel_backup_contact_name,
      merged.channel_backup_contact_phone,
      merged.legal_name,
      merged.legal_id_number,
      merged.legal_phone,
      merged.legal_contact_address,
      merged.legal_email,
      merged.enterprise_backup_name,
      merged.enterprise_backup_phone,
      t,
      id,
    );
    const row = db.prepare(`${CUSTOMER_SELECT} WHERE c.id = ?`).get(id) as CustomerRow;
  ...
  api.delete("/customers/:id", (req, res) => {
    ...
    const cur = db.prepare("SELECT * FROM customers WHERE id = ?").get(id) as CustomerRow | undefined;
    if (!cur) return res.status(404).json({ error: "未找到客户" });
    if (!canAccessRow(req, cur)) return res.status(403).json({ error: "无权删除该客户" });
