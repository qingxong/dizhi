...
function findByOaEntryId(oaEntryId) {
    return db.prepare("SELECT * FROM customers WHERE oa_entry_id = ?").get(oaEntryId);
}
/** 仅同步当前登录用户在 OA 中「销售负责人」为自己的客户 */
export async function syncCustomersFromOa(opts) {
    const result = { imported: 0, updated: 0, skipped: 0, total_fetched: 0, errors: [] };
    if (!opts.oaMemberId) {
    ...
    for (const raw of rows) {
        ...
        WHERE oa_entry_id = ?`).run(mapped.customer_type, mapped.channel_company_name, mapped.channel_common_contact_name, mapped.channel_common_contact_phone, mapped.channel_backup_contact_name, mapped.channel_backup_contact_phone, mapped.legal_name, mapped.legal_phone, mapped.enterprise_backup_name, mapped.enterprise_backup_phone, mapped.oa_customer_sn, t, mapped.oa_entry_id);
            result.updated += 1;
            continue;
        }
        const id = nanoid();
      ...
      ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(id, mapped.customer_type, mapped.channel_company_name, mapped.channel_common_contact_name, mapped.channel_common_contact_phone, mapped.channel_backup_contact_name, mapped.channel_backup_contact_phone, mapped.legal_name, null, mapped.legal_phone, null, null, mapped.enterprise_backup_name, mapped.enterprise_backup_phone, mapped.oa_entry_id, mapped.oa_customer_sn, "oa", ownerId, t, t);
        result.imported += 1;
    }
    if (skipNoPhone > 0) {
        result.errors.push(`共 ${skipNoPhone} 条无对接人电话（子表单第 1 行与历史主表均无有效号码），已跳过`);
    }
    if (skipOther > 0) {
