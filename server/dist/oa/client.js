...
function apiKey() {
    ...
    if (!key)
        throw new Error("未配置 OA_API_KEY，请在服务器环境变量中设置");
    return key;
}
function dataUrl() {
    return `${OA.BASE_URL}/openapi/v1/app/${OA.APP_ID}/entry/${OA.ENTRY_ID}/data`;
}
function oaErrorMessage(json, fallback) {
    const raw = json.message ?? json.msg;
    if (raw && typeof raw === "string")
        return raw;
    if (json.code === 4004)
        return "超出请求频率限制，请稍后再试";
    return fallback;
}
...
/** 分页拉取全部（页间延迟 + 限流重试） */
export async function fetchAllOaCustomers(salesOwnerId) {
    const limit = 300;
    const all = [];
    let skip = 0;
    for (;;) {
